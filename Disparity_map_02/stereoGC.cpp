...
// get cost of an edge in graph
double getCost(const doubleImage& I1,  // Image 1
               const doubleImage& I1M, // Image of mean intensity value over patch
               const doubleImage& I2,  // Image2
               const doubleImage& I2M, // Image of mean intensity value over patch
               int u1, int v1,         // Pixel of interest in image 1
               int u2, int v2,         // Pixel of interest in image 2
               int n,                  // Half patch size
               int wcc){               // discretization factor wcc
    double cost = zncc(I1, I1M, I2, I2M, u1, v1, u2, v2, n);
    cost = (cost < 0) ? 1 : sqrt(1-cost); //non linear relation of ncc and cost
    return wcc*cost;
}
...
int main() {
    ...
    //layers is the sequence from [dmin, dmax], 
    //number of layer should be dmax-dmin+1
    //number of label is number of layer + 1

    int nbLabel = nd+2;                     // number of label
    int nbLayer = nbLabel-1;                // number of layer
    int nbNode_oneLayer = nx*ny;            // number of node in one layer
    int nbNode = nbNode_oneLayer*nbLayer;   // number of node total
    int numNode = 0;                        // numero of node

    Graph<int,int,int> G(0,0);
    G.add_node(nbNode);

    for(int l=0; l<nbLayer; ++l){ //parcour par layer
        int d = dmin+l;
        for(int x=0; x<nx; ++x){ //parcour in one layer
            for(int y=0; y<ny; ++y){
                int u1 = n+zoom*x;
                int u2 = min(u1+d, w2-n-1); //need bounding box judgement
                int v = n+zoom*y;
                int cost = int(getCost(I1, I1M, I2, I2M, u1, v, u2, v, n, wcc)); 
                //first layer, connected from source
                if(l == 0)
                    G.add_tweights(numNode, cost, 0);
                //last layer, connected to sink
                if(l == nbLayer-1)
                    G.add_tweights(numNode, 0, cost);
                //sauf last layer, connected between layers
                else
                    G.add_edge(numNode, numNode+nbNode_oneLayer, cost, 0);
                //all layers, edge between neighbors
                if(x != nx-1 && y != ny-1){
                    G.add_edge(numNode, numNode+1, lambda, lambda);
                    G.add_edge(numNode, numNode+ny, lambda, lambda);
                }
                else{
                    if(x == nx-1 && y != ny-1) //last col of layer
                        G.add_edge(numNode, numNode+1, lambda, lambda);
                    else if(y == ny-1 && x != nx-1) //last row of layer
                        G.add_edge(numNode, numNode+ny, lambda, lambda);
                }
                numNode++;
            }
    ...
    for (int i=0;i<nx;i++) {
        for (int j=0;j<ny;j++) {
            ...
            for(int l=0; l<nbLayer; ++l){
                int numNode = l*nbNode_oneLayer + i*ny+j; //compute numero of node
                // find the label where we cut
                if(G.what_segment(numNode) == Graph<int,int,int>::SINK){
                    D(i,j) = dmin+l;
                    break;
                }
            }
